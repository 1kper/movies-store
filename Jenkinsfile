...
node('workers'){
    ...
    stage('Push'){
       sh "aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin .dkr.ecr.us-west-2.amazonaws.com"
        ...
        sh "docker tag 1kper/movies-store:latest .dkr.ecr.us-west-2.amazonaws.com/1kper/movies-store:latest"
        sh "docker push .dkr.ecr.us-west-2.amazonaws.com/1kper/movies-store:latest"
    }
}
